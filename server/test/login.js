...
export default (test) => {
  test('Should login with existing username and password', (t) => {
    request(app)
      ...
      .end((err, res) => {
        ...
        t.equal(actualBdoy.user.login, 'test', 'Login matches request');
        t.end();
      });
  });

  test('Should fail on request to login with wrong password', (t) => {
    request(app)
      .post('/api/login')
  ...
  test('Should fail tologin with non-existent user', (t) => {
    request(app)
      .post('/api/login')
